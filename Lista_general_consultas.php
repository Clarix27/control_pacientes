<?php 
  ...
  $pdo = Conexion::getPDO();
  $stmt = $pdo->prepare("SELECT t.nombre, t.a_paterno, t.a_materno, tar.folio FROM titular t INNER JOIN tarjeton tar ON t.pk_titular=tar.fk_titular WHERE tar.fk_titular = :idp");
  $stmt->bindParam(':idp', $pk_titular, PDO::PARAM_INT);
  $stmt->execute();
...
<body>
  ...
  <div class="tituloo_container-subtle">
    <h2>LISTA GENERAL DE CONSULTAS DEL TITULAR</h2>
  </div>
